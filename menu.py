...
import setting, sys, hostSide, clientSide
...
def newGameRequest():
    startBtn.configure(text="Crear sala", command=lambda: [ventanaMenu.destroy(), hostSide.main()])
    configBtn.configure(text="Entrar", command=lambda: [ventanaMenu.destroy(), clientSide.main()])
